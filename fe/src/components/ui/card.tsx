...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "flex flex-col gap-5 rounded-2xl border border-border/90 bg-card/95 py-5 text-card-foreground shadow-[0_16px_34px_-24px_rgb(17_45_80/0.42)] backdrop-blur",
        className,
      )}
      {...props}
    />
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex items-center px-5 sm:px-6 [.border-t]:pt-5",
        className,
      )}
      {...props}
    />

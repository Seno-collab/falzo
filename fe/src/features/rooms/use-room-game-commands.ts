...
export function useRoomGameCommands(
    ...
    async (
      ...
      try {
        ...
        if (accessToken && error instanceof ApiError && error.status === 409) {
          try {
            onState(
              await getCurrentRoundState(accessToken, roomId, {
                trackActivity: false,
              }),
            );
            onError("");
  ...
  return {
    pending,
    confirmRole: () =>
      execute("ready", confirmRoomRole, "Không thể xác nhận thẻ vai trò"),
    finishTurn: () =>
      execute("finish", finishRoomTurn, "Không thể kết thúc lượt"),
    castVote: (targetPlayerId: number) =>
      execute(
        "vote",
        (token, activeRoomId) =>
          castRoomVote(token, activeRoomId, targetPlayerId),
